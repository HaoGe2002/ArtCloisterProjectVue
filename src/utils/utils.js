...
//用于获取css属性的值
export function getCssVariable(str) {
    return getComputedStyle(document.documentElement).getPropertyValue(str)
  }
//帮图片添加地址
export function getImageUrl(imageUrl) {
  const base = import.meta.env.VITE_APP_BASE_API; // 例如 "http://localhost:7700"
  // 如果 imageUrl 已经包含 base，则直接返回，否则拼接 base 和 imageUrl
  if (!imageUrl || typeof imageUrl !== "string") {
    return defAva; // ⚠️ 返回默认图片
  }
  if(imageUrl === null){
    return defAva;
  }
